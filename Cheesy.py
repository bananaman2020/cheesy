...
def main():
    setTitle(f"Cheesy V3 {THIS_VERSION}")
    clear()
    global threads
    global cancel_key
    if getTheme() == "cheesyo":
    ...
    if choice == "1":
        ...
        if threading.active_count() < threads:
            threading.Thread(
                target=util.accountNuke.Cheesy_Nuke,
        ...
        if token.startswith("mfa."):
            print(
                f"{Fore.RESET}[{Fore.RED}Error{Fore.RESET}] : Just a headsup Cheesy wont be able to delete the servers since the account has 2fa enabled"
            )
            sleep(3)
        processes = []
        # get all servers
...
{Fore.RESET}[{Fore.LIGHTGREEN_EX}3{Fore.RESET}]{Fore.LIGHTGREEN_EX} HypeSquad Balance
        ...
        print(
            f"{Fore.RED}COMING SOON. . .\n{Fore.RESET}u just gotta wait bro"
        )
        sleep(4)
        main()
...
if __name__ == "__main__":
    ...
    if os.path.basename(sys.argv[0]).endswith("exe"):
        search_for_updates()
        with open(getTempDir() + "\\cheesy_proxies", "w"):
            pass
        if not os.path.exists(getTempDir() + "\\cheesy_theme"):
            setTheme("cheesyo")
        clear()
        proxy_scrape()
        sleep(1.5)
    ...
    try:
        ...
        print(
            f"{Fore.RED}Woopsie daisy, your python version ({sys.version_info[0]}.{sys.version_info[1]}.{sys.version_info[2]}) is not compatible with cheesy v3 nuker, please download python 3.7+"
        )
        sleep(5)
        print("exiting. . .")
        sleep(1.5)
